 ...
 * As of 2026-08-29 all tools use client-only islands + ToolCrawlerFallback for
 * crawlable HTML — keeps view-source lean while preserving SEO text in #tool.
 */
...
const noSsrList = Object.keys(registry).sort();
fs.writeFileSync(
  ssrPolicyFile,
  `/* AUTO-GENERATED by scripts/generate-tool-registry.mjs — do not edit */
export const NO_SSR_TOOL_SLUGS = new Set(${JSON.stringify(noSsrList)});

/** All tools hydrate client-side; server HTML lives in ToolCrawlerFallback + ToolSeoSections. */
export function toolNeedsClientOnly(_slug: string): boolean {
  return true;
}
`
);
...
console.log(
  `Wrote ${outFile}: ${Object.keys(registry).length} tools; loaders: ${keepLoaders.size}; clients: ${keepClients.size} (client-only); pages: ${keepPages.size}; slugs: ${slugsOut}; missing: ${missing.length}`
);

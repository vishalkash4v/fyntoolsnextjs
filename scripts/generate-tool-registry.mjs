...
/**
 * Tools that must stay client-only (camera, getUserMedia, etc.).
 * Everything else is server-rendered with a direct import for Google-visible HTML.
 */
const NO_SSR_TOOLS = new Set([
  "add-name-date-photo",
  "background-remover",
  "barcode-generator",
...
for (const [slug, file] of Object.entries(registry)) {
  ...
  const ssr = !NO_SSR_TOOLS.has(slug);
  const clientCode = ssr
    ? `'use client';
...
const ssrPolicyFile = path.join(root, "src", "lib", "tools", "ssr-policy.generated.ts");
const noSsrList = [...NO_SSR_TOOLS].sort();
fs.writeFileSync(
  ssrPolicyFile,
  `/* AUTO-GENERATED by scripts/generate-tool-registry.mjs — do not edit */
export const NO_SSR_TOOL_SLUGS = new Set(${JSON.stringify(noSsrList)});
...
const ssrCount = Object.keys(registry).filter((s) => !NO_SSR_TOOLS.has(s)).length;
console.log(
  `Wrote ${outFile}: ${Object.keys(registry).length} tools; loaders: ${keepLoaders.size}; clients: ${keepClients.size} (ssr: ${ssrCount}); pages: ${keepPages.size}; slugs: ${slugsOut}; missing: ${missing.length}`
);

...
/**
 * Tier A — SSR the tool island for faster LCP (url-shortener pattern).
 * Exclude File/camera/WASM/canvas-heavy tools (keep ssr:false).
 */
const SSR_TOOLS = new Set([
  "word-counter",
  "password-generator",
  "text-case-converter",
  "meta-tag-previewer",
...
const Tool = dynamic(() => import("@/components/tools/${file}"), {
  ...
  loading: () => (
    <div
      className="w-full min-h-[${ssr ? 420 : 560}px] rounded-xl border border-zinc-200 dark:border-zinc-800 bg-muted/30 animate-pulse flex items-center justify-center text-muted-foreground"
      aria-busy="true"
...
export default function ToolClient() {
  return (
    <div className="w-full min-h-[${ssr ? 420 : 560}px]" id="tool-interface">
      <Tool />
    </div>
  );
}
...
console.log(
  `Wrote ${outFile}: ${Object.keys(registry).length} tools; loaders: ${keepLoaders.size}; clients: ${keepClients.size} (ssr: ${[...SSR_TOOLS].filter((s) => registry[s]).length}); pages: ${keepPages.size}; slugs: ${slugsOut}; missing: ${missing.length}`
);

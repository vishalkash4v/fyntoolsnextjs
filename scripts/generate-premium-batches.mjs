...
for (const tool of tools) {
  ...
  testimonials[tool.path] = testimonialsFor(tool);
}

const outDir = path.join(root, "src/data/tool-content/premium");
fs.mkdirSync(outDir, { recursive: true });

const premiumTs = `/* AUTO-GENERATED by scripts/generate-premium-batches.mjs — re-run to refresh */
...
const testiTs = `/* AUTO-GENERATED by scripts/generate-premium-batches.mjs — curated-style quotes per tool */
...
export const toolTestimonials: Record<string, ToolTestimonial[]> = ${JSON.stringify(testimonials, null, 2)};
`;

fs.writeFileSync(path.join(root, "src/data/tool-content/toolTestimonials.ts"), testiTs);

console.log(
  `Premium tools: ${Object.keys(premium).length}; testimonials paths: ${Object.keys(testimonials).length}`
);

'use client';
/* AUTO-GENERATED by scripts/generate-tool-registry.mjs — do not edit */

import dynamic from "next/dynamic";

const Tool = dynamic(() => import("@/components/tools/AgeCalculator"), {
  ssr: true,
  loading: () => (
    <div
      className="w-full min-h-[420px] rounded-xl border border-zinc-200 dark:border-zinc-800 bg-muted/30 animate-pulse flex items-center justify-center text-muted-foreground"
      aria-busy="true"
      aria-label="Loading Age Calculator"
      role="status"
    >
      Loading tool…
    </div>
  ),
});

/** Dedicated island — only loads AgeCalculator, not the multi-tool registry. */
export default function ToolClient() {
  return (
    <div className="w-full min-h-[420px]" id="tool-interface">
      <Tool />
    </div>
  );
}
